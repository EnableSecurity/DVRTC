...
Capture the SIP and RTP traffic generated by DVRTC's background call generator and use it to inspect call signaling and listen to recovered audio.
...
```bash
./scripts/compose.sh --scenario pbx1 run --rm testing tshark -i any -f 'port 5060 or portrange 10000-15000 or portrange 35000-40000' -a duration:120 -w /work/callgen-traffic.pcap
```

The current call generators place staggered 20-second calls with no pause, so a 120-second capture gives you several chances to catch a full call with both signaling and media. The capture is written to `artifacts/callgen-traffic.pcap`.
...
```bash
wireshark artifacts/callgen-traffic.pcap
```
...
DVRTC runs background baresip call-generator instances that continuously place calls to extension `1300`. Those calls create SIP signaling on port `5060` and RTP media on the configured media port ranges, so there is already traffic available to inspect without generating any new calls yourself.

This exercise works because DVRTC allows plaintext SIP and does not require encrypted media for these call flows. An observer with packet capture access can recover call metadata from SIP and, in many cases, play the captured RTP audio directly in Wireshark.

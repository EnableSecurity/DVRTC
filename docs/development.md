...
```bash
docker compose -f docker-compose.yml -f docker-compose.dev.yml build kamailio
```
...
Run `validate_env.sh` as a pre-flight check before `docker compose up -d` to catch missing variables, certificates, or addressing issues early.
